...
def chooseSeat():
    ...
    submitStart = browser.find_element_by_xpath("/html/body/div[6]/div[3]/button[2]")
    submitStart.click()
    sleep(0.5)
    ...
    ultimateTimeSelect.select_by_visible_text("{}".format(endTime))
...
if __name__ == '__main__':
    ...
    Label(myWindow, text="endTime").grid(row=5)
    ...
    Button(myWindow, text='Quit', command=myWindow.quit).grid(row=6, column=0, sticky=W, padx=5, pady=5)
    Button(myWindow, text='Run', command=chooseSeat).grid(row=6, column=1, sticky=W, padx=5, pady=5)
    # 进入消息循环
    myWindow.mainloop()
